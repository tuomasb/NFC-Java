...
public class UltralightUtilities {
	...
	public boolean readPages(int startPage, int numberOfPages,
		...
		// The address is the number 0...15 of the 4-byte page.
		for (int i = 0; i < numberOfPages; i++) {
			boolean status = ul.readBinary(startPage + i, destination, destinationStartByte + i * 4);
			if (!status) {
	...
	public void printMemory(java.io.PrintStream out) throws CardException {
		...
		for (int i = 0; i < 16; i++) {
			...
			for (int j = 0; j < 4; j++) {
				for (int k = 0; k < 8; k++)
					out.print((((memory[4 * i + j] >> (7 - k)) % 2) == 0 ? 0 : 1));
				out.print(" ");
			}
			out.println();
		}
	...
	public boolean writePages(byte[] source, int sourceStartByte,
			int startPage, int numberOfPages) throws CardException {
		boolean status;
		// We always read and write one 4-byte page at a time.
		// The address is the number 0...15 of the 4-byte page.
		for (int i = 0; i < numberOfPages; i++) {
			status = ul.writeBinary(startPage + i, source, sourceStartByte + 4
					* i);
			if (!status) {
	...
	public boolean writeMemory(byte[] memoryImage, int startpage, int numberOfpages)
	...
	// Zero all pages 4...15. Fails if any one of the pages is locked.
	...
	public boolean lockPage(int pageNumber) throws CardException {
		if (pageNumber < 4 || pageNumber > 15)
			throw new CardException("Page to lock must be between 4 and 15.");
		...
		if (!status) userMessage("Failed locking page" + pageNumber + ".");
		return status;
	}
